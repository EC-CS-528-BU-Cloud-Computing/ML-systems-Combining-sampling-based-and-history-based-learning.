...
class YarnOccupancyStatsEvent(Event):
    ...
    def handle(self):
        ...
        elastic_containers = len(list(task for node in self.state.nodes.values()
                                 for task in node.running_containers if task.is_elastic))
        ...
        next_event.time_millis = self.state.simulator.getClockMillis() + \
            int(self.state.user_config.occupancy_stats_interval) * 1000
        # Create a new event if the collection interval has been set
        if next_event.time_millis > self.state.simulator.getClockMillis():
            self.state.simulator.add_event(next_event)
            self.state.occupancy_stats_event = next_event
