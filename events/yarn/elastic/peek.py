...
class YarnPeekSimulationStartEvent(YarnSimulationStartEvent):
    ...
    def handle(self):
        ...
        next_peek_simulation.time_millis = self.state.simulator.getClockMillis()
...
class YarnPeekSimulationEvent(YarnOracleSimulationEvent):
    ...
    def should_run_simulation(self, cutoff_time):
        ...
        for job in self.state.jobs:
            ...
            if job.trace_start_ms > self.state.simulator.getClockMillis() or job.finished or job.pending_tasks:
                continue
    ...
    def compute_cutoff_time(self):
        return PeekOracle.compute_cutoff_time(self.state.simulator.getClockMillis(),
    ...
    def handle(self):
        ...
        if self.one_time:
            ...
            if next_simulation_event is not None and \
                    next_simulation_event.time_millis == self.state.simulator.getClockMillis():
            ...
            next_simulation.time_millis = self.state.simulator.getClockMillis() + self.state.user_config.node_hb_ms
    ...
    def run_peek_simulation(self, cutoff_time):
        ...
        simulator_state = PeekOracle.generate_simulation(self.state, self.log,
                                                         self.state.simulator.getClockMillis(), cutoff_time)
