...
from utils import SCHEDULING_GOALS
import sys
import random
import time
...
class StatsGenerator(object):
    ...
    def get_job_stats_str(self):
        ...
        for job in self.sorted_jobs:
            result.append("{} start_ms={} end_ms={} duration_s={:.2f} tasks=({})".format(
                ...
                ",".join(["{}MBx{}".format(task.resource.memory_mb, task.num_containers)
    ...
    def get_misc_stats_str(self):
        if self.state.scheduling_goal is SCHEDULING_GOALS.DEADLINE.value:
            print "\n\n\n in get_misc_stats_str: these results might be inaccurate for Deadline case \n\n\n"
...
class StatsWriter(object):
    def __init__(self, state, stats_generator):
        ...
        self.state = state
        self.base_results_directory = self.state.base_results_directory
        self.experiment_identifier = self.state.experiment_identifier

    def write_stats(self):
        temp_file_base_name = self.base_results_directory+self.experiment_identifier
        if len(temp_file_base_name) > 240:
            random.seed(time.time())
            temp_random_string = str(random.randint(1, 999))
            sys.stderr.write("Output filenames can be too long so setting its base value to: "+ temp_random_string+" from: "+ temp_file_base_name+"\n")
            sys.stdout.write("Output filenames can be too long so setting its base value to: "+ temp_random_string+" from: "+ temp_file_base_name+"\n")
 
            temp_file_base_name = temp_random_string+"-" 
        sys.stderr.write("job_stats begin "+"\n")
        sys.stderr.write(self.stats_generator.get_job_stats_str() + "\n")
        sys.stderr.write("job_stats end "+"\n")

        with open(temp_file_base_name+"job_stats.out", "w") as f:
            print "outputs at ", self.base_results_directory+self.experiment_identifier+"job_stats.out"
            f.write(self.stats_generator.get_job_stats_str() + "\n")

        with open(temp_file_base_name+"dec_stats.out", "w") as f:
            f.writelines([self.stats_generator.get_job_decision_stats_str(), "\n",
                          self.state.scheduler.get_total_stats_str(), "\n"])

        with open(temp_file_base_name+"tsk_stats.out", "w") as f:
            f.write(self.stats_generator.get_task_stats_str() + "\n")
            sys.stderr.write("task_stats begin "+"\n")
            sys.stderr.write(self.stats_generator.get_task_stats_str() + "\n")
            sys.stderr.write("task_stats end "+"\n")

        with open(temp_file_base_name+"msc_stats.out", "w") as f:
            f.write(self.stats_generator.get_misc_stats_str() + "\n")
            sys.stderr.write("misc_stats: "+self.stats_generator.get_misc_stats_str() + "\n")

        with open(temp_file_base_name+"res_stats.out", "w") as f:
            f.write(self.stats_generator.get_reservation_stats_str() + "\n")
            sys.stderr.write("reservation_stats: "+self.stats_generator.get_reservation_stats_str() + "\n")
