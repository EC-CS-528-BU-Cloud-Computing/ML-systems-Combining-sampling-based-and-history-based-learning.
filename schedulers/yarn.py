...
from events.yarn.yarn import YarnAMHeartbeatEvent, SLS_AM_ALLOCATION_TO_CONTAINER_REQ_DELAY
...
from utils import PATIENCE_QUEUE_NAMES 

#YarnSchedulerType = PEnum("YarnSchedulerType", "REGULAR GREEDY SMARTG SYMBEX RACE_LOCKSTEP RACE_CONTINUOUS " +
#                          "RACE_JOB RACE_NODEG SRTF PEEK PATIENCE") # Any change here please update this declaration in models/yarn/objects.py too. As this is copied there too.

LOG = logging.getLogger("yarn_schedulers")

ERROR_RANDOM_SEED = 24381092348021
YARN_MIN_ALLOCATION_MB = 1  #AJ_code changed this to 1 to test on the gTrace, it's default value was 100. Find out its usage.
print "Changed YARN_MIN_ALLOCATION_MB to 1 to test on the gTrace it's default value was 100. Find out its usage."
...
class YarnScheduler(YarnSchedulerStats):
    ...
    def __init__(self, state):
        ...
        self.allocated_containers = {}  # this will contain the containers that were allocated in the scheduling phase. job_id and list of containers.Containers ready to run
        self.next_job_id = 1
        ...
        for node_id in self.state.nodes:
            self.state.nodes[node_id].set_state(self.state)
    ...
    def do_heartbeat_creating_on_job_arrival(self, job):    #   This was created for jobs which are part of dag but no YarnJobArriveEvent is created for them and they are considered arrived upon arrival/sampling completion of their parent
    ...
    def all_jobs_are_done(self):
        return all((job.finished or job.dropped_on_deadline_miss or job.not_admitted) for job in self.state.jobs)
    ...
    def handle_job_arrived(self, job):
        ...
        for job_itr in jobs_to_itr:
            ...
            if job_itr in self.running_jobs:
                LOG.error("Attempt to launch existing job: " + str(job_itr))
                raise Exception("Attempt to launch existing job")


            # Adjust job start time
            job_itr.start_ms = self.state.simulator.getClockMillis()
            self.stats_job_start_update(job_itr.start_ms)

            # Add job to set of running jobs
            self.running_jobs.add(job_itr)
            ...
            if self.state.user_config.mem_error is not None:
                self.mem_errors[job_itr.job_id] = self.compute_memory_error_adjustment(
                    next(task for task in job_itr.tasks if task.type != YarnContainerType.MRAM)
                )

    def handle_job_completed(self, job):
        # Update finishing time
        job.end_ms = self.state.simulator.getClockMillis()
    ...
    def create_container_from_task(self, node, allocated, job, task, tillQueueJumpOnly = False):
        err_adjusted_duration = duration = task.getRemainingDuration()  # this is to handle the case when a task has been partially executed by jumping queues and now is being assigned in a main queue. However, the time to trigger queue jump event has still to be calculated in the YarnNodeContainerQueueJumpEvent.
        ...
        yarn_container = YarnRunningContainer(
            ...
            node=node,
            task=task,
            toScheduleTillQueueJumpOnly=tillQueueJumpOnly)

        yarn_container.duration_error = err_adjusted_duration - duration

        return yarn_container

    def handle_container_allocation(self, node, allocated, job, task, time_millis, tillQueueJumpOnly = False):
        # Create a container from the task
        if job.job_id == utils.DEBUG_JID[0]:
            sys.stderr.write("In handle_container_allocation for job_id: "+str(job.job_id)+" time_millis: " + str(time_millis)+"\n")
        yarn_container = self.create_container_from_task(node, allocated, job, task, tillQueueJumpOnly = tillQueueJumpOnly)
        # Adjust job to reflect launched container
        task.num_containers -= 1
        task.setSchedulingQueue(job.get_job_current_queue())
        if self.state.scheduler_type is YarnSchedulerType.PATIENCE:
            #if job.get_job_current_queue() == PATIENCE_QUEUE_NAMES.SAMPLE.value:
                task.setSchedulingSubQueue(job.get_job_current_subqueue())
#            else:
#                print "I came there." +str(job.get_job_current_queue()) + " job: "+str(job.job_id)
        ...
        #if job.job_id == 6391113248:sys.stderr.write("job_id: "+str(6391113248) + ", width: "+str(len(job.tasks)) + ", node.get_temp_node_type: " + str(node.get_temp_node_type()) + ", tillQueueJumpOnly: "+ str(tillQueueJumpOnly)+ ", node_id: "+str(node.node_id)+", task_type: "+ str(yarn_container.task.type)+ ", num_running_containers: "+ str(len(node.running_containers)) + ", num_allocated_containers: "+ str(len(node.allocated_containers))+"\n")
...
class YarnFairScheduler(YarnScheduler):
    ...
    def adjust_for_allocation(self, node, task, queue_idx, alloc_result):
        ...
        self.handle_container_allocation(node, allocated_resource, job, task,
                                         self.state.simulator.getClockMillis())
    ...
    def schedule(self, node, queue_start=0):
        ...
        if node.reserved_application is not None:
            ...
            if required_task is None or \
                ...
                reserved_duration = self.state.simulator.getClockMillis() - node.reserved_start_ms
        ...
        while queue_idx < len(self.job_queue):
            ...
            if alloc_result is not None and \
                ...
                node.reserved_start_ms = self.state.simulator.getClockMillis()
                self.stats_reserve_node(job.job_id, node.node_id, self.state.simulator.getClockMillis())
                break
            else:
                self.stats_reject_decisions_inc(job.job_id)
                queue_idx += 1
...
class YarnRegularScheduler(YarnFairScheduler):
    ...
    def create_container_from_task(self, node, allocated, job, task, tillQueueJumpOnly = None):
        yarn_container = super(YarnRegularScheduler, self).create_container_from_task(node, allocated, job, task)
        if LOG.isEnabledFor(logging.DEBUG):
            LOG.debug("YARN_REGULAR_SCHEDULER: " + str(self.state.simulator.getClockMillis()) + " Allocated " +
