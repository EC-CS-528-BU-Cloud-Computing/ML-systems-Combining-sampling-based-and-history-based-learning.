...
from utils import parsenumlist, get_enumparser, SCHEDULING_GOALS, PATIENCE_PARTITION_TUNNING, PATIENCE_COMMON_QUEUE
...
def main():
    ...
    parser.add_argument("--scheduling-goal", type=int,
                        default=SCHEDULING_GOALS.DEFAULT, help="scheduling can be done with the goal of different metric like average JCT, deadline etc. Please see the SCHEDULING_GOALS class in the utils.")
    ...
    patience_args.add_argument("--auto-tunable-partition", type=int,
                              help="if set to one then the share of nodes of type sampling, thin and main are determined on the fly.", default=PATIENCE_PARTITION_TUNNING.DEFAULT)
    patience_args.add_argument("--common-queue", type=int,
                               default=PATIENCE_COMMON_QUEUE.DEFAULT)
    ...
    parser.add_argument("--job-arrival-time-stretch-multiplier", type=float, help="The scale for stretching the arrival time in the given input trace.", default=1) ## JS_add
