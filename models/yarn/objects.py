...
#                          "RACE_JOB RACE_NODEG SRTF PEEK PATIENCE") #   AJ_code # Any change here please update this declaration in schedulers/yarn.py too. As this is copied from there.
...
class YarnResource(object):
    ...
    #LOG.warn("Currently comparing only v.cores for scheduling policy resource consideration. Following 7 functions changed for that. If changing this remove log from run function in simulator.py")
    def __add__(self, other):
        #return YarnResource(self.memory_mb + other.memory_mb, self.vcores + other.vcores)
        return YarnResource(self.memory_mb + 0, self.vcores + other.vcores)
...
class YarnNode(object):
    ...
    def book_container(self, container):
        ...
        if self.state.scheduler_type == YarnSchedulerType.PATIENCE: #AJ_code
            self.update_task_wise_resource_usage(container.task, allocate = True, finish = False)    #AJ_code
            self.update_state_resource_dict(container.task, allocate = True, finish = False)   #AJ_code
...
class YarnContainer(object):
    def __init__(self, resource=None, priority=None, container_type=None, job=None, duration=0, penalty=None):
        ...
        self.tid_within_job = None  #   AJ_code #   Filled while creating the job in the YarnPrototypeContainer and later in the YarnRunningContainer.
...
class YarnRunningContainer(YarnContainer):
    def __init__(self, container_id=0, resource=None, priority=None, container_type=None, job=None,
                 node=None, task=None, duration=0, toScheduleTillQueueJumpOnly = False):
        ...
        self.finish_event = None    #   AJ_code
        self.launch_event = None    #   AJ_code
        self.is_evicted = False     #   AJ_code
        self.to_schedule_till_queue_jump_only = toScheduleTillQueueJumpOnly #   AJ_code
        self.next_queue_jump_event = None   #   AJ_code
        self.tid_within_job = self.task.tid_within_job    #   AJ_code
    ...
    def __deepcopy__(self, memo):
        ...
        new_container.to_schedule_till_queue_jump_only = self.to_schedule_till_queue_jump_only
    ...
    def setNextQueueJumpEvent(self, event):  #   AJ_code
        if not self.toScheduleTillQueueJumpOnly():
            raise Exception("Container: "+str(self)+" not to be scheduled till queue jump only but still setNextQueueJumpEvent called")
        self.next_queue_jump_event = event

    def getNextQueueJumpEvent(self):    #   AJ_code
        return self.next_queue_jump_event

    def setFinishEvent(self, event):    #   AJ_code
        self.finish_event = event
 
    def setLaunchEvent(self, event):    #   AJ_code
        self.launch_event = event

    def getFinishEvent(self):    #   AJ_code
        return self.finish_event
 
    def getLaunchEvent(self):    #   AJ_code
    ...
    def toScheduleTillQueueJumpOnly(self):
        return self.to_schedule_till_queue_jump_only

    def schedule_container(self, time_millis):
        self.scheduled_time_millis = time_millis
        self.job.on_container_schedule(self, time_millis)
        if self.job.get_sampling_start_ms() == INVALID_VALUE and self.job.get_job_current_queue() == PATIENCE_QUEUE_NAMES.SAMPLE.value:
            self.job.set_sampling_start_ms(time_millis)

    def deschedule_container(self):
        if self.finished_time_millis == -1: #   if finished_time_millis != -1 it implies that the container is finished and is being evicted just because of some event trails.
            self.scheduled_time_millis = -1
            self.launched_time_millis = -1

    def takeEvictionAction(self, current_time):
        self.is_evicted = True
        if self.launched_time_millis > current_time:
            raise Exception("In YarnRunningContainer.takeEvictionAction container.launched_time_millis is greater than eviction time")
        if self.launched_time_millis > 0 and self.task.type is not YarnContainerType.MRAM:
            if self.finished_time_millis == -1: #   if finished_time_millis != -1 it implies that the container is finished and is being evicted just because of some event trails.
                execution_time = current_time - self.launched_time_millis
                self.task.increaseTimeExecutedSoFar(execution_time)
        if self.task.isSamplingTask():
            if not self.job.sampling_over():
                self.job.decrease_sampling_tasks_assigned(1)
            
        self.deschedule_container()

    def isEvicted(self):
        return self.is_evicted

    def launch_container(self, time_millis):
        self.launched_time_millis = time_millis

    def finish_container(self, time_millis):
        #print("Container {}:{} Finished.".format(self.job.name, self.id))
    ...
    def remaining_duration_ms_from_now(self, current_simulator_time_millis):    #AJ_code # this function assumes that task will not be evicted now.
        #raise Exception("From function YarnJob.remaining_duration_ms_from_now: Check in what context this function is being called and see how is the value returned being used and check correctness of this: expected_finish_time = current_simulator_time_millis + self.duration in the function.")
        #   Currently the function is using the exact duration. However for the
        #   purpose of prediction in all-or-none it should be using the sampled
        #   duration. Make that edit after first testing.
        #expected_finish_time = current_simulator_time_millis + self.duration
        #if expected_finish_time < self.launched_time_millis:
        #    raise Exception("Container {}:{}. In function remaining_duration_ms_from_now expected_finish_time < self.launched_time_millis .".format(self.job.name, self.id))
        #if self.launched_time_millis == -1: # This means that container will be launched in atmost next am_hb_ms
        #    expected_finish_time += self.job.am_hb_ms
        #    return  expected_finish_time - current_simulator_time_millis    # assuming container will be launced immediately now. Since we have already incremented the expected_finish_time by self.job.am_hb_ms this assumption is not creating any problem. 
        #elif self.finished_time_millis == -1:
        #    return  expected_finish_time - self.launched_time_millis
        #else:
        #    return 0
        
        if self.task.type is YarnContainerType.MRAM:
            return 0
 
        if self.launched_time_millis == -1:
            return self.task.duration
        elif self.finished_time_millis == -1:
            expected_finish_time = self.launched_time_millis + self.task.duration
            #expected_finish_time = (self.getFinishEvent()).time_millis
            if expected_finish_time < current_simulator_time_millis:
                if self.node.state.execution_mode == YARN_EXECUTION_TYPES.SIMULATION.value:
                    raise Exception("in YarnJob.remaining_duration_ms_from_now. expected_finish_time: "+str(expected_finish_time) + " current_simulator_time_millis: "+ str(current_simulator_time_millis) + " self.launched_time_millis: "+ str(self.launched_time_millis) + " self.task.duration: " +str(self.task.duration) + " predicted_finish_time: "+ str(self.launched_time_millis + self.task.duration)+ " minimum_finish_delay: " + str(self.launched_time_millis + self.task.duration - expected_finish_time))
                else:
                    sys.stderr.write("POTENTIAL_EXCEPTION: in YarnJob.remaining_duration_ms_from_now. expected_finish_time: "+str(expected_finish_time) + " current_simulator_time_millis: "+ str(current_simulator_time_millis) + " self.launched_time_millis: "+ str(self.launched_time_millis) + " self.task.duration: " +str(self.task.duration) + " predicted_finish_time: "+ str(self.launched_time_millis + self.task.duration)+ " minimum_finish_delay: " + str(self.launched_time_millis + self.task.duration - expected_finish_time)+"\n")
            return expected_finish_time - current_simulator_time_millis
        else:
            return 0

    def get_time_remaining_in_current_sub_queue(self):  # returns will_finish_in_this_subqueue(True/False), time remaining in this queue
        if self.node.state.scheduler_type != YarnSchedulerType.PATIENCE:
            raise Exception("get_time_remaining_in_current_sub_queue is meant to be called only from PATIENCE")
        try:
            current_subqueue_max_runtime, thin_limit = self.node.state.scheduler.get_runtime_for_queue_and_sub_queue(self.task.getSchedulingQueue(), self.task.getSchedulingSubQueue())
        except TypeError:
            print "type(self.node.state.scheduler): ",type(self.node.state.scheduler)
 
        #if self.job.get_job_initial_number_of_tasks() >= thin_limit:
        #    current_subqueue_max_runtime = float("inf")
        #else:
        #    sys.stderr.write("For a thin job: "+str(self.job.job_id)+" time , address assigned is: "+str(current_subqueue_max_runtime)+". Please change this if the limit for the first main sub queue is not Inf.")
            #Read the sys.stderr.write below.
        #sys.stderr.write("assigning Inf runtime for wide sampling jobs and according to queue jump for thin jobs, the function self.node.state.scheduler.get_runtime_for_queue_and_sub_queue now returns second number which is thin limit. This is valid only till the first main subqueue limit is also Inf\n")

        task_remaining_duration = self.task.getRemainingDuration()
        will_finish_in_this_subqueue = False
        if task_remaining_duration < current_subqueue_max_runtime:
            will_finish_in_this_subqueue = True
        return will_finish_in_this_subqueue, min(current_subqueue_max_runtime, task_remaining_duration)

    def act_on_queue_jump_event(self):  # This function is called when the task has needs to be moved out of a subqueue i.e. when it has finished its portion of execution in that queue.
        if self.launched_time_millis == -1:
            raise Exception("Container {}:{} not launched yet but act_on_queue_jump_event called".format(self.job.name, self.id))
        self.task.increaseTimeExecutedSoFar(self.node.state.simulator.getClockMillis() - self.launched_time_millis)
        self.node.remove_container(self)
        self.node.state.scheduler.decrement_autotune_nodetype_count_stats_for_node(self.node, "YarnRunningContainer.act_on_queue_jump_event")
        self.job.consumption -= self.resource
        self.job.running_tasks.remove(self)
        #print "act_on_queue_jump_event"
        if self.task not in self.job.pending_tasks:
            #print "act_on_queue_jump_event going to append task"
            self.job.pending_tasks.append(self.task)
            if self.node.state.auto_tunable_partition != PATIENCE_PARTITION_TUNNING.MANUAL.value:
                self.node.state.scheduler.increment_unscheduled_tasks_for_job_by(self.job, 1)
            if self.task.isSamplingTask():
                if not self.job.sampling_over():
                    self.job.decrease_sampling_tasks_assigned(1)
        self.task.num_containers += 1
        job_queue = self.job.get_job_current_queue()
        job_subqueue = self.job.get_job_current_subqueue()
        task_queue = self.task.getSchedulingQueue()
        task_subqueue = self.task.getSchedulingSubQueue()
        if job_queue == PATIENCE_QUEUE_NAMES.THIN.value:
            if self.state.common_queue != PATIENCE_COMMON_QUEUE.OFF.value:
                raise NotImplementedError()
            self.node.state.scheduler.update_job_queue_and_subqueue(self.job, task_queue, task_subqueue)
        elif job_queue == PATIENCE_QUEUE_NAMES.SAMPLE.value:
            self.node.state.scheduler.update_job_queue_and_subqueue(self.job, task_queue, task_subqueue)
        elif job_queue == PATIENCE_QUEUE_NAMES.MAIN.value:
            if self.node.state.common_queue == PATIENCE_COMMON_QUEUE.OFF.value: 
                self.node.state.scheduler.if_needed_add_job_in_main_queue(self.job)
            elif self.node.state.common_queue == PATIENCE_COMMON_QUEUE.THIN_MAIN.value:
                if self.job.get_width(self.node.state.scheduler.width_type) < self.node.state.thin_limit:
                    self.node.state.scheduler.update_job_queue_and_subqueue(self.job, task_queue, task_subqueue)
                else:
                    self.node.state.scheduler.if_needed_add_job_in_main_queue(self.job)
            else:
...
class YarnPrototypeContainer(YarnContainer):
    def __init__(self, resource=None, priority=None, container_type=None, job=None, duration=0, num_containers=0,
                 ideal_resource=None, penalty=None, tid_within_job=None):
        ...
        self.is_sampling_task = False    #   AJ_code
        self.evicted_count = 0  #   AJ_code
        self.scheduling_queue = INVALID_VALUE   #   AJ_code
        self.time_executed_so_far = 0   #   AJ_code
        self.scheduling_sub_queue = INVALID_VALUE   #   AJ_code
        self.tid_within_job = tid_within_job    #AJ_code
    ...
    def __deepcopy__(self, memo):
        ...
        new_container.scheduling_queue = self.scheduling_queue   #   AJ_code
        new_container.evicted_count = self.evicted_count 
        new.tid_within_job = self.tid_within_job    #AJ_code
        return new_container
    ...
    def markSamplingTask(self):  #   AJ_code
        self.is_sampling_task = True 

    def markIsNotSamplingTask(self):  #   AJ_code
        if self.is_sampling_task:
            pass
            #print "\n\n\n\n\n\n\n\Marking not sampling: ", self.job.job_id, "\n\n\n\n\n\n\n\n"
        self.is_sampling_task = False 

    def isSamplingTask(self):   #   AJ_code
        return self.is_sampling_task

    def takeEvictionAction(self): #   AJ_code
        self.evicted_count += 1

    def setSchedulingQueue(self, queue_to_set): #   AJ_code
        self.scheduling_queue = queue_to_set

    def getSchedulingQueue(self):   #   AJ_code
        return self.scheduling_queue

    def setSchedulingSubQueue(self, sub_queue_to_set):  #   AJ_code
        #print "In setSchedulingSubQueue for task: "+str(self)+" to sub_queue_to_set: " +str(sub_queue_to_set)
        self.scheduling_sub_queue = sub_queue_to_set

    def getSchedulingSubQueue(self):   #   AJ_code
        return self.scheduling_sub_queue
    
    def increaseTimeExecutedSoFar(self, by): #   AJ_code
        self.time_executed_so_far += by
        #print "increaseTimeExecutedSoFar by: ", by, " self.time_executed_so_far: ", self.time_executed_so_far
        if self.getTimeExecutedSoFar() > self.duration:
            raise Exception("Container {}:{}. ".format(self.job.name, self.tid_within_job)+str(self)+ " has duration = "+str(self.duration) + " but has been executed for: "+ str(self.time_executed_so_far))
 
    def getTimeExecutedSoFar(self): #   AJ_code
        #if self.state.eviction_policy != PATIENCE_EVICTION_POLICY.NO.value:
        #    raise Exception("Function YarnPrototypeContainer.getTimeExecutedSoFar might not be working perfectly with other eviction policies. Please verify it.")

        return self.time_executed_so_far
 
    def getRemainingDuration(self): #   AJ_code
        temp_time_executed_so_far = self.getTimeExecutedSoFar()
        if temp_time_executed_so_far > self.duration:
            raise Exception(str(self)+ " has duration = "+str(self.duration) + " but has been executed for: "+ str(temp_time_executed_so_far))
        return self.duration - temp_time_executed_so_far
    
    def getEstimatedRemainingDuration(self, oracle_state): #   AJ_code
        if self.job.get_job_current_queue() != PATIENCE_QUEUE_NAMES.MAIN.value and oracle_state != PATIENCE_ORACLE_STATE.THREE_SIGMA.value:
            raise Exception("getEstimatedRemainingDuration is supposed to be called for jobs in main queue only. However called for a job in queue: "+str(self.job.get_job_current_queue()))
        temp_time_executed_so_far = self.getTimeExecutedSoFar()
        if temp_time_executed_so_far > self.duration:
            raise Exception(str(self)+ " has duration = "+str(self.duration) + " but has been executed for: "+ str(temp_time_executed_so_far))
        
        estimated_duration = self.job.estimates.get_estimated_running_time(oracle_state)

        return max(0, (estimated_duration - temp_time_executed_so_far))
