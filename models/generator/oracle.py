    ...
    def generate_oracle_runner(state, log, simulator_clock, scheduler_type, simulation_type):
        ...
        while True:
            try:
                ...
                #new_state.simulator.getClockMillis() = simulator_clock
                new_state.simulator.setClockMillis(simulator_clock)
                deepcopy_memo[id(state.simulator)] = new_state.simulator

                # JOBS
                # noinspection PyArgumentList
                new_state.jobs = list(copy.deepcopy(job, deepcopy_memo) for job in state.scheduler.running_jobs)
                ...
                if scheduler_type is YarnSchedulerType.REGULAR or \
                    ...
                    new_scheduler.running_jobs = set(new_state.jobs)  # This works since we only copy running jobs.
