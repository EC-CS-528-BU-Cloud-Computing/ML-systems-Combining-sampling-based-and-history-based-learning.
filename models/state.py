...
from utils import YARN_EXECUTION_TYPES
...
class YarnState(State):
    def __init__(self, simulator=None, user_config=None, nodes=None, racks=None, generator=None, scheduler=None,
        ...
        self.overall_cummulative_time_wise_resource_availability_dict = {}  #AJ_code -- maintain the invariant that only nodes can update this. Time here should be maintained in absolute sense.
    ...
    def isInCoordinatorExecMode(self):  #   AJ_code #   TESTBED
        if self.execution_mode == YARN_EXECUTION_TYPES.COORDINATOR.value:
            return True
        elif self.execution_mode == YARN_EXECUTION_TYPES.SIMULATOR.value:
            return False
        else:
            raise Exception("Unidentified execution mode: "+str(self.execution_mode))
 
    def get_job_object_by_jid(self, jid):   #   AJ_code
        return self.jid_job_dict[jid]

    def assertCoordinatorExecMode(self):
        if not self.isInCoordinatorExecMode():
            raise Exception("Expected coordinator exec mode but current mode is: "+str(self.execution_mode))
